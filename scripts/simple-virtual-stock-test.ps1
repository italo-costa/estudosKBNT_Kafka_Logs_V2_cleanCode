        ...
        switch ($op) {
            ...
            "UPDATE" {
                if ($createdStocks.Count -gt 0) {
                    ...
                    Invoke-RestMethod -Uri "$baseUrl/stocks/$stockId/quantity" -Method PUT -ContentType "application/json" -Body ($data | ConvertTo-Json)
            ...
            "RESERVE" {
                if ($createdStocks.Count -gt 0) {
                    ...
                    Invoke-RestMethod -Uri "$baseUrl/stocks/$stockId/reserve" -Method POST -ContentType "application/json" -Body ($data | ConvertTo-Json)
            ...
            "QUERY" {
                if ($createdStocks.Count -gt 0) {
                    $stockId = $createdStocks | Get-Random
                    Invoke-RestMethod -Uri "$baseUrl/stocks/$stockId" -Method GET
                } else {
                    Invoke-RestMethod -Uri "$baseUrl/stocks" -Method GET
                }
