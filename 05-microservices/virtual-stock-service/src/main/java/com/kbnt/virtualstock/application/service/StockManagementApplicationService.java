...
public class StockManagementApplicationService implements StockManagementUseCase {
  ...
  private static final org.slf4j.Logger log = 
      org.slf4j.LoggerFactory.getLogger(StockManagementApplicationService.class);

  public StockManagementApplicationService(StockRepositoryPort stockRepository, 
      StockEventPublisherPort eventPublisher) {
  ...
  public StockCreationResult createStock(CreateStockCommand command) {
    try {
      ...
      // Create domain event
      StockUpdatedEvent event = StockUpdatedEvent.forCreation(savedStock, 
      ...
      log.error("Error creating stock for product: {}", 
          command.getProductId().getValue(), e);
  ...
  public StockUpdateResult updateStockQuantity(UpdateStockQuantityCommand command) {
    try {
      ...
      if (stockOpt.isEmpty()) {
        return StockUpdateResult.failure("Stock not found: " 
            + command.getStockId().getValue());
      }

      Stock currentStock = stockOpt.get();
      Stock updatedStock = currentStock.updateQuantity(command.getNewQuantity(), 
      ...
      log.info("Stock quantity updated successfully for stock ID: {} from {} to {}", 
          command.getStockId().getValue(), currentStock.getQuantity(), 
          savedStock.getQuantity());

      return StockUpdateResult.success(savedStock, event);

    } catch (Exception e) {
      log.error("Error updating stock quantity for stock ID: {}", 
          command.getStockId().getValue(), e);
      return StockUpdateResult.failure("Failed to update stock quantity: " 
          + e.getMessage());
    }
  }
  @Override
  public StockUpdateResult updateStockPrice(UpdateStockPriceCommand command) {
    try {
      ...
      Stock currentStock = stockOpt.get();
      Stock updatedStock = currentStock.updatePrice(command.getNewPrice(), 
          command.getUpdatedBy());
      ...
      log.info("Stock price updated successfully for stock ID: {} from {} to {}", 
          command.getStockId().getValue(), currentStock.getUnitPrice(), 
          savedStock.getUnitPrice());

      return StockUpdateResult.success(savedStock, event);

    } catch (Exception e) {
      log.error("Error updating stock price for stock ID: {}", 
          command.getStockId().getValue(), e);
  ...
  public StockReservationResult reserveStock(ReserveStockCommand command) {
    try {
      ...
      Optional<Stock> stockOpt = stockRepository.findById(command.getStockId());
      if (stockOpt.isEmpty()) {
        return StockReservationResult.failure("Stock not found: " 
            + command.getStockId().getValue());
      }
      ...
      if (!currentStock.canReserve(command.getQuantityToReserve())) {
        return StockReservationResult.failure(
            String.format("Cannot reserve %d units. Available: %d, Status: %s", 
                command.getQuantityToReserve(), currentStock.getQuantity(), 
                currentStock.getStatus()));
      }

      Stock updatedStock = currentStock.reserve(command.getQuantityToReserve(), 
      ...
      log.info("Stock reserved successfully - {} units of stock ID: {} reserved, "
          + "remaining: {}", command.getQuantityToReserve(), 
          command.getStockId().getValue(), savedStock.getQuantity());

      return StockReservationResult.success(savedStock, event, 
          command.getQuantityToReserve());

    } catch (Exception e) {
      log.error("Error reserving stock for stock ID: {}", 
          command.getStockId().getValue(), e);
      return StockReservationResult.failure("Failed to reserve stock: " 
          + e.getMessage());
    }
  }
  @Override
